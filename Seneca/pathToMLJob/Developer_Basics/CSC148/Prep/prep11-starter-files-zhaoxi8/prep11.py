...
def mergesort3(lst: List) -> List:
    ...
    if len(lst) < 2:  # We've provided the base case for you.
        ...
        triple = len(lst) // 3
        lst_1_sorted = mergesort(lst[:triple])
        lst_2_sorted = mergesort(lst[triple:2*triple])
        lst_3_sorted = mergesort(lst[2*triple:])

    return merge3(lst_1_sorted, lst_2_sorted, lst_3_sorted)
...
def merge3(lst1: List, lst2: List, lst3: List) -> List:
    ...
    while index1 < len(lst1) and index2 < len(lst2):
        if lst1[index1] <= lst2[index2]:
            compared.append(lst1[index1])
            index1 += 1
        else:
            compared.append(lst2[index2])
            index2 += 1

    tmp_lst = compared + lst1[index1:] + lst2[index2:]
    return _merge(tmp_lst, lst3)
...
def kth_smallest(lst: List, k: int) -> Any:
    ...
    10
    >>> kth_smallest([10, 20, -4, 3, -5, 7, 11], 6)
    20
    ...
    if len(lst) < 2:
        ...
        if len(smaller) > k:
            ...
            return kth_smallest(bigger, k - 1 - len(smaller))
